...
export default function AllAud() {
  ...
  const handleDownloadAudio = async (id: string) => {
    try {
      const token = await getToken();
      if (token) {
        await dispatch(downloadAudio({ id, token }));
        showToast("Downloding", "success");
      }
    } catch (error) {
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6">
      {fetchLoading ? (
      ...
      ) : !audioData?.Result || audioData.Result.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[400px]">
          <div className="p-4 bg-purple-50 rounded-full mb-4">
            <FiMusic size={48} color="#7E22CE" />
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
          {audioData.Result.map((file: FileData) => (
            <div
              key={file._id}
                ...
                <div className="relative menu-container">
                  ...
                  {openMenuId === file._id && (
                    <div className="absolute right-0 mt-2 w-36 sm:w-40 md:w-48 bg-white rounded-lg shadow-lg py-1 z-10">
                      ...
                      <button
                        className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm text-gray-700 hover:bg-gray-100 flex items-center space-x-2"
                        onClick={() => handleDownloadAudio(file._id)}
                      >
                        <FiDownload size={14} />
