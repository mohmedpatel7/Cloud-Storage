...
export default function AllDocs() {
  ...
  const documentsData = fileUpload?.getAllDoc as unknown as
    | GetAllDataResponse
    | undefined;
  ...
  const fetchData = async () => {
    try {
      ...
      if (token) {
        await dispatch(getAllDocuments(token));
      }
    } catch (error) {
  ...
  const handleDeleteDocument = async (id: string) => {
    try {
      ...
      const token = await getToken();
      if (token) {
        await dispatch(deleteDocument({ id, token }));
        await fetchData();
  ...
  const handleDownloadDocument = async (id: string) => {
    try {
      const token = await getToken();
      if (token) {
        await dispatch(downloadDocument({ id, token }));
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6">
      {fetchLoading ? (
      ...
      ) : !documentsData?.Result || documentsData.Result.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[400px]">
          <div className="p-4 bg-green-50 rounded-full mb-4">
            <FiFileText size={48} color="#16A34A" />
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4">
          {documentsData.Result.map((file: FileData) => (
            <div
              key={file._id}
            ...
            >
              <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
                <div className="p-2 sm:p-2.5 md:p-3 bg-green-50 rounded-lg">
                  <FiFileText size={18} color="#16A34A" />
                </div>
                ...
                <div className="relative menu-container">
                  ...
                  {openMenuId === file._id && (
                    <div className="absolute right-0 mt-2 w-36 sm:w-40 md:w-48 bg-white rounded-lg shadow-lg py-1 z-10">
                      ...
                      <button
                        className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm text-gray-700 hover:bg-gray-100 flex items-center space-x-2"
                        onClick={() => handleDownloadDocument(file._id)}
                      >
                        <FiDownload size={14} />
              ...
              <div className="relative z-10">
                ...
                <div className="flex gap-4 mt-8">
                  <button
                    ...
                    onClick={() =>
                      fileProp && handleDownloadDocument(fileProp._id)
                    }
                  >
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (fileProp) {
                        await handleDeleteDocument(fileProp._id);
                        closeModal();
                      }
                    }}
                  >
